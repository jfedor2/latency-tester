...
static uint8_t init3[] = { 0x06, 0x20, 0x00, 0x02, 0x01, 0x00 };
...
static void process_setup(struct xdev_t* xdev) {
    switch (xdev->setup_stage) {
        ...
        case 4:
            ...
            uint16_t vid;
            uint16_t pid;
            tuh_vid_pid_get(xdev->dev_addr, &vid, &pid);
            descriptor_received_callback(vid, pid, xbox_one_descriptor, sizeof(xbox_one_descriptor), (uint16_t) (xdev->dev_addr << 8) | xdev->itf_num);
